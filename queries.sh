...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games AS winner_games ON teams.team_id = winner_games.winner_id FULL JOIN games AS opp_games ON teams.team_id = opp_games.opponent_id WHERE (winner_games.year=2014 AND winner_games.round='Eighth-Final') OR (opp_games.year=2014 AND opp_games.round='Eighth-Final') ORDER BY name;")"
